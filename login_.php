...
if (isset($_POST['Submit'])) {
  ...
  if (mysqli_num_rows($result_check) == 0) {
    ...
    if (($db_age['category'] == "Admin" || $db_age['category'] == "Finance") && $inf_employee == "No") {
      ...
      } else if ($post == "SAOAAdmin-2-2025" && $caste == "OBC") {
        if ($disability == "Yes" && $disability_percentage >= 40 && $caste != "Ex-servicemen" && $type_of_job == "Permanent") {
          ...
          $db_age_new = $db_age['age_limit'] + $length_of_service_new + 26;
        } else if ($disability == "No" && $caste != "Ex-servicemen" && $type_of_job != "Permanent") {
    ...
    if ($agenew1 > 56 && $disability == "Yes") {
      ...
      // 1) read all four current paths once
      $stmt = $link->prepare(
        "SELECT caste_certi, disability_certi, stenography_certi, typing_certi
   FROM prsnl WHERE id = ?"
      );
      ...
      // 2) initialise every variable with the value already in the DB
      $certi_file_path        = $row['caste_certi'];
      $disability_certi_path  = $row['disability_certi'];
      $stenography_certi_path = $row['stenography_certi'];
      $typing_certi_path      = $row['typing_certi'];
      ...
      try {
        $mail->SMTPDebug = 0;
        $mail->isSMTP();
        $mail->Host = 'smtp.zeptomail.in';           // Changed
        $mail->SMTPAuth = true;
        $mail->Username = 'emailapikey';              // Changed
        $mail->Password = 'Zoho-enczapikey PHtE6r0EQuHrjzF98EQF7P+8HsGkMIp79eIyKgQWsIcRCqMHHU0Dqt4plWS/+Rp5BqYQF/Oewdps5+jJ4b+AIWfrN2ZPCWqyqK3sx/VYSPOZsbq6x00bsV4dfkbaXYHrc9Zo0iHRu9jSNA==';  // Changed
        $mail->SMTPSecure = 'tls';
        $mail->Port = 587;
        $mail->setFrom('noreply@mail.inflibnet.ac.in', 'INFLIBNET Centre');  // Changed (no hyphen)
        ...
        // Optionally log error: error_log($mail->ErrorInfo);
      }
    }
  } else {
        ...
        if (mysqli_num_rows($result) > 0) {
          while ($row = mysqli_fetch_assoc($result)) {
            ...
            try {
              ...
              $mail->Host = 'smtp.zeptomail.in';           // Changed from smtp.office365.com
              $mail->SMTPAuth = true;
              $mail->Username = 'emailapikey';              // Changed from email address
              $mail->Password = 'Zoho-enczapikey PHtE6r0EQuHrjzF98EQF7P+8HsGkMIp79eIyKgQWsIcRCqMHHU0Dqt4plWS/+Rp5BqYQF/Oewdps5+jJ4b+AIWfrN2ZPCWqyqK3sx/VYSPOZsbq6x00bsV4dfkbaXYHrc9Zo0iHRu9jSNA==';  // API Key
              $mail->SMTPSecure = 'tls';
              $mail->Port = 587;
              $mail->setFrom('noreply@mail.inflibnet.ac.in', 'INFLIBNET Centre');  // Changed (no hyphen in noreply)
              // ==============================================

              $mail->addAddress($email);
              $mail->isHTML(true);

              // Optional: Add these from ZeptoMail example if needed
              ...
              $message = '<table cellpadding="10" width="100%" border="1">
                        ...
                        Your password has been reset by the system.<br/>
                        Your new password is : <b>' . $new_password . '</b>
                        <br/>Please login with your application ID(<b>' . $app_id . '</b>) and new password.</td>
                        </tr>
                        <tr> <td style="border:none;"> </td> </tr>
                        <tr> <td style="border:none;"> <b>Note:</b> This is an auto generated email. In case of any technical queries please contact: recruitment[at]inflibnet[dot]ac[dot]in</td> </tr>
                        <tr>
              ...
              $_SESSION['forget_msg'] = '<div class="text-center alert alert-danger">Mail Error: ' . $e->getMessage() . '</div>';
              header("Location:forget_password.php");
              exit;
            }
          }
