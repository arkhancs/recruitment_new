...
/* === NEW : server-side gap check === */
for ($i = 1; $i <= 10; $i++) {
  $org  = trim($_POST['org' . $i]  ?? '');
  $pos  = trim($_POST['pos' . $i]  ?? '');
  $from = trim($_POST['from' . $i] ?? '');
  $to   = trim($_POST['to' . $i]   ?? '');

  if ($org == '' && $pos == '' && $from == '' && $to == '') continue; // blank tail ok
  if ($org == '' || $pos == '' || $from == '' || $to == '') {
    echo json_encode(['status' => 'incomplete_row', 'row' => $i]);
    exit;
  }
}
/* === end NEW === */
...
$expected = [];
for ($i = 1; $i <= 10; $i++) {
  $expected['org' . $i]  = $_POST['org' . $i]  ?? '';
  $expected['pos' . $i]  = $_POST['pos' . $i]  ?? '';
  $expected['from' . $i] = $_POST['from' . $i] ?? '';
  $expected['to' . $i]   = $_POST['to' . $i]   ?? '';
}

$actual = [];
$sql = "SELECT org1,org2,org3,org4,org5,org6,org7,org8,org9,org10,
        ...
        FROM exprn WHERE id='$reg_id'";
$res = mysqli_query($link, $sql);
$row = mysqli_fetch_assoc($res);

/* === production verification that tolerates escaping === */
$match = true;
foreach ($expected as $col => $expVal) {
  $expVal = mysqli_real_escape_string($link, $expVal);
  $actVal = $row[$col] ?? '';
  if ($expVal !== $actVal) {
    $match = false;
    break;
