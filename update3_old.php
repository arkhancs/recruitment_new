...
//$sql_file = "select exp_file1, exp_file2, exp_file3, exp_file4, exp_file5, exp_file6, exp_file7, exp_file8, exp_file9, exp_file10 from exprn where id='$reg_id'";
//$result_file = mysqli_query($link, $sql_file);
//$expr_file_path = '';
//$sql_file = "select expr_certi from exprn where id='$reg_id'";
//$result_file = mysqli_query($link, $sql_file);
//if (mysqli_num_rows($result_file) == 0) {
//    
//} else {
//    while ($row = mysqli_fetch_assoc($result_file)) {
//        $expr_file_path = $row['expr_certi'];
//    }
//}
//if ($_FILES["expr_certi"]["name"]) {
//    $temp = explode(".", $_FILES["expr_certi"]["name"]);
//    $newfilename = $reg_id . '-expr_certi.' . end($temp);
//    $expr_file_path = 'uploads/expr/' . $newfilename;
//    move_uploaded_file($_FILES["expr_certi"]["tmp_name"], $expr_file_path);
//}
...
for ($i = 1; $i <= 10; $i++) {
    ...
    if ($_POST[$org_data] != '') {
        ...
        if ($_FILES["$temp_filename"]["name"]) {
            ...
            $sql = "update exprn set org" . $i . "='" . $_POST['org' . $i] . "',pos" . $i . "='" . $_POST['pos' . $i] . "',from" . $i . "='" . $_POST['from' . $i] . "', to" . $i . "='" . $_POST['to' . $i] . "', nature" . $i . "='" . $_POST['nature' . $i] . "',"
                . "pay" . $i . "='" . $_POST['pay' . $i] . "', otype" . $i . "='" . $_POST['otype' . $i] . "', exp" . $i . "='" . $_POST['exp' . $i] . "', exp_file" . $i . "='$expr_file_path',"
                . "total1 = '$total1', total2 = '$total2', total3 = '$total3', total4 = '$total4', total5 = '$total5', grandtotal = '$grandtotal', expr_certi = '', currently_working='$currently_working'"
                . "where id = '$reg_id'";
        } else {
            //            if (mysqli_num_rows($result_file) == 0) {
            //                
            //            } else {
            //                while ($row = mysqli_fetch_assoc($result_file)) {
            //                    $exp_file . $j = $row['exp_file' . $i];
            //                }
            //            }
            //            $expr_file_path = $exp_file . $j;

            $sql = "update exprn set org" . $i . "='" . $_POST['org' . $i] . "',pos" . $i . "='" . $_POST['pos' . $i] . "',from" . $i . "='" . $_POST['from' . $i] . "', to" . $i . "='" . $_POST['to' . $i] . "', nature" . $i . "='" . $_POST['nature' . $i] . "',"
                . "pay" . $i . "='" . $_POST['pay' . $i] . "', otype" . $i . "='" . $_POST['otype' . $i] . "', exp" . $i . "='" . $_POST['exp' . $i] . "',"
                . "total1 = '$total1', total2 = '$total2', total3 = '$total3', total4 = '$total4', total5 = '$total5', grandtotal = '$grandtotal', expr_certi = '', currently_working='$currently_working'"
                . "where id = '$reg_id'";
        }
...
$sql = "select * from exprn join othrs on exprn.id = othrs.id where exprn.id = '$reg_id'";
