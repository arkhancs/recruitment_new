...
if (isset($_POST['Submit'])) {
    ...
    $sql = "select p.id as app_id, p.*, e.*, ep.*, o.*, re.closed_date, re.year from prsnl p left join edctn e on p.id = e.id left join exprn ep on p.id = ep.id left join othrs o on p.id = o.id left join req_experience re on re.post = p.post where p.id ='$regid' and p.password = '$password'";
...
//$date = date("d/m/Y");
    ...
    if (mysqli_num_rows($result_check) == 0) {
        ...
        if (($caste == "SC" || $caste == "ST" || $caste == "OBC") && ($db_age['category'] != "Admin")) {
            ...
            if ($db_age_new == $agenew1) {
                if (strpos($agenew, 'and') !== false) {
                    ...
                    if ($result1) {
                        ...
                        $sql4 = "insert into othrs VALUES('$unid','', '','','','','','','','','','','','','','','','','','','','','','','')";
...
//Recipients
    ...
    $sql = "update prsnl set password='$enc_password' where email='$email' and post='$post'";
    $result = mysqli_query($link, $sql);
